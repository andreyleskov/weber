defmodule Weber.Tests.Unit.Synonym do
  ...
  test "synonym can be added to a word" do
    assert %Word.Events.SynonymAdded{word: "dish", synonym: "pizza"} ==
        Word.execute(%Word{normalForm: "dish"}, %Word.Commands.AddSynonym{word: "dish", synonym: "pizza"})
  end

  @tag :unit
  @tag :synonym
  test "Given synonym added When appled Then synonym is stored in aggregate" do
    newState =  Word.apply(%Word{normalForm: "dish"},  %Word.Events.SynonymAdded{word: "dish", synonym: "pizza"})
    assert %Word{normalForm: "dish", synonyms: ["pizza"]} == newState
  ...
  test "synonym cannot be added to notmatched world" do
   catch_error Word.execute(%Word{normalForm: "word"}, %Word.Commands.AddSynonym{word: "wrongWord", synonym: "pizza"})
  end

  @tag :unit
  @tag :synonym
  test "synonym cannot be added twice to the same word" do
    word = %Word{normalForm: "dish"}
